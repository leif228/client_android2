...
public class DownGroupAdapter extends BaseAdapter {
	...
	private List<GroupData> mAppList;
...
//    .build(); 
	...
	public DownGroupAdapter(Context c, List<GroupData> shipCooordDatas) {
...
//		mImageLoader.init(imageLoaderConfiguration);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView != null) {
			...
			convertView = mInflater.inflate(R.layout.zd_ship_item1, null);
			holder = new buttonViewHolder();
			holder.item_birth =(TextView)convertView.findViewById(R.id.item_birth);
			convertView.setTag(holder);
		}

		GroupData appInfo = mAppList.get(position);
		if (appInfo != null) {
...
//					holder.shipLogo,MyshipAdapter.displayImageOptions);
			holder.item_birth.setText(appInfo.getGroupName());
