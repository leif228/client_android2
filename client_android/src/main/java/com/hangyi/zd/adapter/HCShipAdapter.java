...
public class HCShipAdapter extends BaseAdapter {
...
//    .build(); 
	...
	public HCShipAdapter(Context c, List<String> shipNames) {
		this.mAppList = shipNames;
		this.mContext = c;
...
//		mImageLoader.init(imageLoaderConfiguration);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView != null) {
			...
			convertView = mInflater.inflate(R.layout.zd_ship_item, null);
			holder = new buttonViewHolder();
			holder.item_birth =(TextView)convertView.findViewById(R.id.item_birth);
			convertView.setTag(holder);
		}

		String appInfo = mAppList.get(position);
		if (appInfo != null) {
...
//					holder.shipLogo,MyshipAdapter.displayImageOptions);
			holder.item_birth.setText(appInfo);
