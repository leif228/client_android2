...
public class PoliceAdapter extends BaseAdapter {
	...
	private List<PoliceData> mAppList;
...
//    .build(); 
	...
	public PoliceAdapter(Context c, List<PoliceData> pds) {
		this.mAppList = pds;
		this.mContext = c;
...
//		mImageLoader.init(imageLoaderConfiguration);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView != null) {
			...
			convertView = mInflater.inflate(R.layout.zd_police_item, null);
			holder = new buttonViewHolder();
			holder.item_birth =(TextView)convertView.findViewById(R.id.item_birth);
			convertView.setTag(holder);
		}

		PoliceData appInfo = mAppList.get(position);
		if (appInfo != null) {
...
//					holder.shipLogo,MyshipAdapter.displayImageOptions);
			NodeCode nc = NodeCode.getByN(Integer.valueOf(appInfo.getEventType()));
			if(null != nc){
//				convertView.setBackgroundColor(R.color.police_r);
				
				if(appInfo.isbRead()){
					holder.item_birth.setTextColor(mContext.getResources().getColor(android.R.color.black));
					holder.item_birth.setText(nc.getDescription().substring(0, 2)+":"+appInfo.getShipName()+"("+appInfo.getEventTime()+")");
//					holder.item_birth.setTextColor(mContext.getResources().getColor(android.R.color.holo_red_light));
//					holder.item_birth.setText(nc.getDescription()+"："+appInfo.getShipName()+"("+appInfo.getEventTime()+")");
				}else{
					holder.item_birth.setTextColor(mContext.getResources().getColor(android.R.color.holo_red_dark));
					holder.item_birth.setText(nc.getDescription().substring(0, 2)+":"+appInfo.getShipName()+"("+appInfo.getEventTime()+")");
//					holder.item_birth.setTextColor(mContext.getResources().getColor(android.R.color.holo_red_dark));
//					holder.item_birth.setText("*"+nc.getDescription()+"："+appInfo.getShipName()+"("+appInfo.getEventTime()+")");
				}
