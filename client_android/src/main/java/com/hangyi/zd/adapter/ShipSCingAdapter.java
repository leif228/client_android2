...
public class ShipSCingAdapter extends BaseAdapter {
	...
	private ArrayList<ShipVoyageData> mAppList;
	...
	public ShipSCingAdapter(Context c, ArrayList<ShipVoyageData> dataList) {
...
//		mImageLoader.init(imageLoaderConfiguration);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView != null) {
			...
			convertView = mInflater.inflate(R.layout.eyd_shipmoniter_item2, null);
...
//			holder.shipId.setText((String) appInfo.get("shipId"));
			holder.shipName.setText(appInfo.getShipName());
...
//			holder.mmsi.setText((String) appInfo.get("MMSI"));
			if(!appInfo.getNodes().isEmpty()){
				NodeCode n = NodeCode.getByN(Integer.valueOf(appInfo.getNodes().get(appInfo.getNodes().size()-1).getStage()));
				String time = appInfo.getNodes().get(appInfo.getNodes().size()-1).getOpTime();
				String des = n!=null?n.getDescription():"";
				holder.carrier.setText("动态："+time+" "+des);
			}
			holder.place.setText("出发港："+appInfo.getStartPort()+" 到达港："+appInfo.getEndPort());
