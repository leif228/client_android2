...
public class LoadImgRunnable3 implements Runnable {
	...
	private StartTimeEndTimeData data;
	private Messenger client;

	public LoadImgRunnable3(Messenger client,  String shipID, Data_loader dataLoader, List<String> channels, StartTimeEndTimeData data) {
		this.client = client;
...
//								mBinderService.startThreadSaveImg(sir);
			...
			if(imgNumTemp != 0)
				synchronized(threadLock) {
					try {
						threadLock.wait();
						Message msg = Message.obtain(null, ImgDownLoadService2.SENDTO_CLIENT, startTimeEndTimeMins, 0);
