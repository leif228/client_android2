...
public class ShipDynamicFragment extends Fragment implements OnClickListener,
	...
	private static int ParseJsonFailure = 6;
...
//		img33.setOnTouchListener(new MyOnTouchListener(3,img33));
	...
	private void initGpsDataArr2() {
		...
		int size = GpsDataQueue.getInstance().size();
...
//				tv_lat3.setText("航向：未知");
				...
				img31.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_gpsBitmap, null, time));
				img32.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_gpsBitmap, null, time));
				img33.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_gpsBitmap, null, time));
			}else{
				shipImg.setVisibility(View.VISIBLE);
...
//				tv_lat4.setText("航向：未知");
				...
				for (int i = 1; i <= size; i++) {
					...
					view.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_gpsBitmap, null, time));
				}
			}
...
//					tv_lat3.setText("航向："+curShip.getGpsCourse());
					...
					if(imgStr31 !=null )
					{
						Bitmap b = AppAdapter.createDrawable2(getActivity(), BitmapFactory.decodeFile(imgStr31), null, curShip.getGpsTime());
...
//						img31.setImageDrawable(AppAdapter.createDrawable(getActivity(), BitmapFactory.decodeFile(imgStr31), null, curShip.getGpsTime()));
						...
						img31.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_imgBitmap, null, curShip.getGpsTime()));
					
					String imgStr32 = getUrl(curShip,channels.get(1));
					if(imgStr32 !=null )
					{
						Bitmap b = AppAdapter.createDrawable2(getActivity(), BitmapFactory.decodeFile(imgStr32), null, curShip.getGpsTime());
...
//						img32.setImageDrawable(AppAdapter.createDrawable(getActivity(), BitmapFactory.decodeFile(imgStr32), null, curShip.getGpsTime()));
						...
						img32.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_imgBitmap, null, curShip.getGpsTime()));
						
					String imgStr33 = getUrl(curShip,channels.get(2));
					if(imgStr33 !=null )
					{
						Bitmap b = AppAdapter.createDrawable2(getActivity(), BitmapFactory.decodeFile(imgStr33), null, curShip.getGpsTime());
...
//						img33.setImageDrawable(AppAdapter.createDrawable(getActivity(), BitmapFactory.decodeFile(imgStr33), null, curShip.getGpsTime()));
						...
						img33.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_imgBitmap, null, curShip.getGpsTime()));
...
//					tv_lat4.setText("航向："+curShip.getGpsCourse());
					...
					for (int i = 1; i <= size; i++) {
						...
						if(imgStr21 !=null )
						{
							Bitmap b = AppAdapter.createDrawable2(getActivity(), BitmapFactory.decodeFile(imgStr21), null, curShip.getGpsTime());
...
//							view.setImageDrawable(AppAdapter.createDrawable(getActivity(), BitmapFactory.decodeFile(imgStr21), null, curShip.getGpsTime()));
							...
							view.setImageDrawable(AppAdapter.createDrawable(getActivity(), lose_imgBitmap, null, curShip.getGpsTime()));
					}
				}
	...
	private void loadShipCKHX(final String start,final String end) {
		...
		dataLoader.getZd_ApiResult(new AsyncHttpResponseHandler() {
			...
			public void onSuccess(final String arg2) {
				...
				parseJsonThread1 = new Thread(new Runnable() {
...
//							Util.writeFileData(start+end, arg2);
				...
				parseJsonThread1.setDaemon(true);
				parseJsonThread1.start();
			}
			@Override
			public void onFailure(Throwable error, String content) {
				super.onFailure(error, content);
...
//		NewContentFragment.mapAddPort(getActivity(),bmHistory);
	...
	public void onDestroy() {
		...
		if(parseJsonThread1!=null){
			parseJsonThread1.interrupt();
			parseJsonThread1 = null;
		}
