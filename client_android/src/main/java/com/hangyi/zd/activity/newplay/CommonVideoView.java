...
public class CommonVideoView extends FrameLayout implements
	...
	public static boolean isNotLoadedPlayData = true;
	...
	public interface CommonVideoChangLintener{
		...
		boolean loadPlayGpsData(Calendar start, Calendar end);
	}
	...
	public void init(Calendar startc, Calendar endc) {
		startTime.setText(initDateTime(startc));
		endTime.setText(initDateTime(endc));
		
		onInitState();
		isNotLoadedPlayData = true;
	}
	public void initProgressBar(int ps) {
		videoSeekBar.setMax(ps);
	...
	private void initView() {
		...
		startTime.setText(initDateTime(null));
		endTime.setText(initCurrTime());
...
//	}
	...
	private Calendar getCalendarByTextView(TextView v){
		String s = (String) v.getText();
		return DateTimePickDialogUtil.getCalendarByInintData(s);
	}
	...
	public void onClick(View v) {
		...
		case R.id.videoPauseImg:
			if(isNotLoadedPlayData){
				boolean flag = commonVideoChangLintener.loadPlayGpsData(getCalendarByTextView(startTime),getCalendarByTextView(endTime));
				if(flag){
					isNotLoadedPlayData = false;
					onCacheState();
				}else{
					return;
				}
...
//			}
	...
	// 初始化开始时间
	private String initDateTime(Calendar c) {
