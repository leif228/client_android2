...
public class ShipDuesPaymentActivity extends FragmentActivity {
		...
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					Toast.makeText(ShipDuesPaymentActivity.this, "支付成功",Toast.LENGTH_SHORT).show();
					Intent intent = new Intent(ShipDuesPaymentActivity.this,ShipDuesActivity.class);
					...
					if (TextUtils.equals(resultStatus, "8000")) {
						Toast.makeText(ShipDuesPaymentActivity.this, "支付结果确认中",
								Toast.LENGTH_SHORT).show();

					} else {
						Toast.makeText(ShipDuesPaymentActivity.this, "支付失败",
								Toast.LENGTH_SHORT).show();
			...
			case SDK_CHECK_FLAG: {
				if("true".equalsIgnoreCase((String)msg.obj)){
					Toast.makeText(ShipDuesPaymentActivity.this, "您已经安装支付宝客户端，可以正常支付",Toast.LENGTH_SHORT).show();
				}else{
					Toast.makeText(ShipDuesPaymentActivity.this, "请先安装支付宝客户端，才能完成支付",Toast.LENGTH_SHORT).show();
				}
				break;
			}
			default:
		...
		payBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(payType.equalsIgnoreCase("2")){
					...
					intent.setClass(ShipDuesPaymentActivity.this, ShipDuesPayActivity.class);
	...
	public void pay(View v) {
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(ShipDuesPaymentActivity.this);
				// 调用支付接口
	...
	public void check(View v) {
		Runnable checkRunnable = new Runnable() {
			...
			@Override
			public void run() {
				PayTask payTask = new PayTask(ShipDuesPaymentActivity.this);
	...
	public String getOrderInfo() {
		...
		if(payData !=null){
			...
			// 服务器异步通知页面路径
			orderInfo += "&notify_url=" + "\"" + ApplicationConstants.NOTIFY_URL_SHIPDUES
					+ "\"";
			// 接口名称， 固定值
