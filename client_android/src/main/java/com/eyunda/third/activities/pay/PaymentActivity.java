...
public class PaymentActivity extends FragmentActivity {
	...
	AsyncHttpResponseHandler showHandler = new AsyncHttpResponseHandler() {
		...
		public void onFailure(Throwable arg0, String content) {
			...
			Toast.makeText(PaymentActivity.this, content,
					Toast.LENGTH_SHORT).show();
		}
		@Override
		public void onStart(){
			dialogUtil = new DialogUtil(PaymentActivity.this);
		...
		public void onSuccess(String res) {
			...
			if(cd.getReturnCode().equalsIgnoreCase("success")){
				...
				payData = new PayEntity(var);
				setView();
				dialog.dismiss();
			}else{
				dialog.dismiss();
				Toast.makeText(PaymentActivity.this, cd.getMessage(),Toast.LENGTH_SHORT).show();
			}
		}
	};
		...
		public void handleMessage(Message msg) {
			...
			case SDK_CHECK_FLAG: {
				if("true".equalsIgnoreCase((String)msg.obj)){
					Toast.makeText(PaymentActivity.this, "您已经安装支付宝客户端，可以正常支付",Toast.LENGTH_SHORT).show();
				}else{
					Toast.makeText(PaymentActivity.this, "请先安装支付宝客户端，才能完成支付",Toast.LENGTH_SHORT).show();
				}
				break;
			}
			default:
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		uri = (String) bundle.getString("uri");
		payType = (String) bundle.getString("payType");
		...
		payBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(payType.equalsIgnoreCase("2")){
					...
					intent.setClass(PaymentActivity.this, PayActivity.class);
					Bundle bundle = new Bundle();
	...
	private void getData(){
		//异步读取订单信息，当前登录者信息，读取完毕后修改btn
		Map<String, Object> apiParams = new HashMap<String, Object>();
		apiParams.put("id", Long.parseLong(orderId));

		dataLoader.getApiResult(showHandler, "/mobile/order/myOrder/"+uri+"Order",apiParams, "get");
	}
	/**
	...
	public String getOrderInfo() {
		...
		if(payData !=null){
			...
			// 服务器异步通知页面路径
			orderInfo += "&notify_url=" + "\"" + ApplicationConstants.ALIPAY_NOTIFY_URL
					+ "\"";
			// 接口名称， 固定值
