 ...
 *
 */
public class OrderDownloadActivity extends CommonListActivity implements
		OnClickListener {
	...
	private String savePath = Environment.getExternalStorageDirectory()
			.getAbsolutePath() + "/eyunda/download/order/";
	...
	public void onClick(View v) {
		if (v.getId() == R.id.download_img) {// 生成文件并下载
			...
			// 打开文件
			openFile(file, OrderDownloadActivity.this);
		}
		...
		public void run() {
			try {
				...
				URL url = new URL(ApplicationConstants.SERVER_URL
						+ "/mobile/order/myOrder/downloadPdf?id=" + orderId
						+ "&sessionId=" + sessionId);
	...
	private void showError(final String msg) {
		runOnUiThread(new Runnable() {
			public void run() {
				...
				Toast.makeText(OrderDownloadActivity.this, msg, 1).show();
				// finish();
			}
		});
	}
