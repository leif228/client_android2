...
public class JDAdverView extends LinearLayout {
	...
	public void start() {
		...
		if (!isStarted && mAdapter.getCount() > 1) {
			isStarted = true;
			handler.postDelayed(mRunnable, mGap);// 间隔mgap刷新一次UI
		}
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		mPaint.setTextSize(TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_SP, TEXTSIZE, getResources()
						.getDisplayMetrics()));
		mPaint.setStyle(Paint.Style.STROKE);
		canvas.drawText("航讯快报", TEXTSIZE, getHeight() * 2 / 3, mPaint);// 写文字2/3的高度
	}
