...
//		}
		...
		String toUrl = ApplicationConstants.ZDPHP_PRE_URL + apiUrl;
		if (method.equalsIgnoreCase("download")) {
			asyncHttpClient.download(toUrl, params, handler);
		} else if (method.equalsIgnoreCase("post")) {
			asyncHttpClient.post(toUrl, params, handler);
...
//			return;
//		}
		asyncHttpClient.get(ApplicationConstants.ZDJAVA_PRE_URL + ApplicationUrls.version, handler);
	}
	...
	public void uploadBug(AsyncHttpResponseHandler handler,
		...
		asyncHttpClient.post(ApplicationConstants.ZDJAVA_PRE_URL
